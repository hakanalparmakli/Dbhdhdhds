document.addEventListener('DOMContentLoaded', () => {
    ...
    let popularMusicAbortController = null;
    ...
    async function showHomePage() {
        if (popularMusicAbortController) {
            popularMusicAbortController.abort();
        }
        popularMusicAbortController = new AbortController();
        const signal = popularMusicAbortController.signal;

        searchContainer.classList.add('hidden');
        homeLink.classList.add('active');
        searchLink.classList.remove('active');
        libraryLink.classList.remove('active');
        contentArea.innerHTML = `<h2>${currentTranslations.popular_music || 'Popüler Müzikler'}</h2>`;

        try {
            const response = await fetch('/popular', { signal });
            if (!response.ok) throw new Error('Popüler müzikler alınamadı.');
            const results = await response.json();

            // Only update the content if the request was not aborted
            if (!signal.aborted) {
    ...
    function showLibraryPage() {
        ...
        contentArea.innerHTML = `<h2>${currentTranslations.my_library || 'Kitaplığım'}</h2>`;
    ...
    async function performSearch() {
        ...
        showSearchPage();
        contentArea.innerHTML = `<h2>${currentTranslations.search_results || 'Arama Sonuçları'}</h2>`;
