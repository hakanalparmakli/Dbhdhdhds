...
test.describe('H-Sound Settings Page', () => {
    ...
    test('should allow a logged-in user to navigate to and view the settings page', async ({ page }) => {
        ...
        // Step 5: Play a song and open the modal
        await page.click('.result-item'); // Click the first song to play it
        await page.waitForSelector('#player-footer:not(.hidden)');
        await page.click('#song-thumbnail'); // Click the album art to open the modal

        await expect(page.locator('#video-modal')).not.toHaveClass(/hidden/);
        await expect(page.locator('#modal-player')).toBeVisible();

        // Step 6: Take a screenshot of the modal
